...
L'analyse Wireshark confirme le bon fonctionnement du scénario d'authentification HTTP : affichage de la page de login, connexion réussie avec redirection vers le dashboard, déconnexion, puis tentative échouée avec message d'erreur. Le trafic observé met en évidence que les identifiants sont transportés en clair en HTTP, ce qui justifie la migration vers HTTPS et le renforcement des mesures de sécurité côté application.
...
## 3. MÉTHODOLOGIE DE CAPTURE WIRESHARK
...
### 3.3 Démarche de test

1. Ouverture de l'URL `http://192.168.3.7/index.php` depuis le client.
2. Tentative de connexion valide (`admin` / mot de passe valide).
3. Accès au dashboard puis déconnexion.
4. Tentative de connexion invalide.
5. Arrêt de la capture et analyse des requêtes/réponses HTTP.
...
2. **Après authentification réussie**
   ...
   - Interface affichée : page privée "Authentification réussie" avec message de bienvenue et lien de déconnexion
...
3. **Après authentification échouée**
   ...
   - Capture associée (référence principale) : Annexe C
...
Lors de la phase initiale, la table `users` pouvait être vide ou mal configurée, ce qui provoquait un échec d'authentification. Après correction, il faut conserver au moins un utilisateur valide pour les tests.
...
Les annexes ci-dessous documentent les interfaces applicatives, les observations réseau et l'état du serveur.
...
### Annexe G : Capture client additionnelle 1

![Capture client additionnelle 1](assets/5f238136-e836-491c-9f39-54b35a830471.jpeg)

### Annexe H : Capture client additionnelle 2

![Capture client additionnelle 2](assets/e9921ec1-f52a-43c1-a8ab-4c02894418c4.jpeg)

### Annexe I : Capture client additionnelle 3

![Capture client additionnelle 3](assets/fa75b434-2236-4209-b48e-33fcf526d422.jpeg)
